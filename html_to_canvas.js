...
(function(window, document, undefined){
    ...
    _html2canvas.Util.Bounds = (function getBounds () {
        var _cache=null,_counter=0

        return function getBounds(el){if(el===false){_cache=null;return}

            if(!_cache){_cache={}
                var l=document.getElementsByTagName("*"),ln=l.length;
                for(var i=0,e;e=l[i],i<ln;i++ ){
                    if(e&&e.nodeType===1&&e.getBoundingClientRect){
                        var id=e.id||(e.id=("anon_"+(++_counter)));
                        var b=e.getBoundingClientRect()
                        if(id){_cache[id]={height:b.height,width:b.width,top:b.top,left:b.left,right:b.right,bottom:b.bottom}}
                    }
                };
            }
            var b=el.id?_cache[el.id]:null;
            if(!b&&(el&&el.nodeType===1&&el.getBoundingClientRect)){
                el.id||(el.id=("anon_"+(++_counter)));
                b=el.getBoundingClientRect()
                //_cache[el.id]={height:b.height,width:b.width,top:b.top,left:b.left,right:b.right,bottom:b.bottom}
            }
            if(b){return {height:b.height,width:b.width,top:b.top,left:b.left,right:b.right,bottom:b.bottom}}
            return {}
        }
    })();
    _html2canvas.Util.Bounds2 = function getBounds2 (el) {                   if(!el){return}
        var clientRect,
            bounds = {};
    ...
    _html2canvas.Util.getCSS = function (el, attribute, index) {
        ...
        function toPX( attribute, val ) {
            ...
            if ( !/^-?[0-9]+\.?[0-9]*(?:px)?$/i.test( val ) && /^-?\d/.test( val ) ) {
                ...
                // Put in the new values to getEntity a computed value out
                if ( rsLeft ) {
    ...
    _html2canvas.Parse = function (images, options) {
        window.scroll(0,0);
        _html2canvas.Util.Bounds(false)
        ...
        var _txtcache={},_wrapElement=null
        function textWrapperBounds(oldTextNode) {
            //var t=oldTextNode.nodeValue
            //if(t&&_txtcache[t]){
            //return _txtcache[t]
            //}
            if(_wrapElement===null){_wrapElement=doc.createElement('span')}
            var parent = oldTextNode.parentNode,
                wrapElement = _wrapElement.cloneNode(),//doc.createElement('wrapper'),
                backupText = oldTextNode.cloneNode(true);

            wrapElement.appendChild(oldTextNode.cloneNode(true));
            parent.replaceChild(wrapElement, oldTextNode);

            var b = _html2canvas.Util.Bounds(wrapElement);
            parent.replaceChild(backupText, wrapElement);
            // if(b.height&&b.width){
            // _txtcache[t]={height:b.height,width:b.width,top:b.top,left:b.left,right:b.right,bottom:b.bottom}
            //  }
            return b;
        }
        ...
        function parseElement (el, stack, pseudoElement) {
            var bnds=_html2canvas.Util.Bounds
            if (bnds(el).height) {//isElementVisible(el)
                stack = renderElement(el, stack, pseudoElement) || stack;
                if (!ignoreElementsRegExp.test(el.nodeName)) {
                    _html2canvas.Util.Children(el).forEach(function(node) {
                        if (node.nodeType === 1 && bnds(node).height) {//node.offsetHeight
    ...
    _html2canvas.Preload = function( options ) {
        ...
        function getImages (el) {
            ...
            try {
                var arr=[].concat(_html2canvas.Util.Children(el)||[]).forEach(function(img) {
                    if(img){getImages(img);}
                });
            }
            catch( e ) {}
            ...
            if (elNodeType === 1 || elNodeType === undefined) {
                ...
                try {
                    ...
                    h2clog("html2canvas: failed to getEntity background-image - Exception: " + e.message);
                }
    ...
    function downloadImg(strData ,strMimeType, strFileName ) {
        var D = document,
            A = arguments,
            a = D.createElement("a"),
            d = A[0],
    ...
    function grabScreen(fn,opts) {fn=typeof(fn)=="function"?fn:function(){}
        var optns={
            logging: true,
            useCORS: true,
            onrendered: function (canvas) {
                ...
                fn( {
                    canvas:canvas,
                    url:img,
                    download:downloadImg.bind(this,img, "image/jpeg") ,  //filename as arg
                    preview:function(url){window.open(url,"screenshot")}.bind(this,img)   //filename as arg
                } )

            }  }
        if(opts){Object.keys(opts).forEach(function(k){optns[k]=opts[k]});  }
        self.html2canvas([document.body], optns);

    }
    window.html2canvas.download=downloadImg
    window.html2canvas.grabScreen=grabScreen
    //download:download /* end download() */
